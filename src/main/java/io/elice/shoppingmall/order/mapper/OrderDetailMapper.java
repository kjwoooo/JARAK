...
public interface OrderDetailMapper {
    ...
    @Mapping(source = "order.id", target = "orderId") // OrderDetail 엔티티의 order 필드의 id 값을 OrderDetailDTO의 orderId 필드에 매핑
    @Mapping(source = "item.id", target = "itemId") // OrderDetail 엔티티의 item 필드의 id 값을 OrderDetailDTO의 itemId 필드에 매핑
    OrderDetailDTO orderDetailToOrderDetailDTO(OrderDetail orderDetail);

    // OrderDetailDTO를 OrderDetail 엔티티로 매핑하는 메서드
    @Mapping(source = "orderId", target = "order.id") // OrderDetailDTO의 orderId 필드 값을 OrderDetail 엔티티의 order 필드의 id에 매핑
    @Mapping(source = "itemId", target = "item.id") // OrderDetailDTO의 itemId 필드 값을 OrderDetail 엔티티의 item 필드의 id에 매핑
