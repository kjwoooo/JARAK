...
public class AddressController {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "회원의 주소 조회 실패", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    })
    @GetMapping("/addresses/{id}")
    public ResponseEntity<AddressResponseDTO> getMemberAddress(@AuthenticationPrincipal UserDetails userDetails,
        @Parameter(name = "id", description = "조회하려는 주소의 id", in = ParameterIn.PATH) @PathVariable Long id){
    ...
    public ResponseEntity<AddressResponseDTO> postMemberAddress(@AuthenticationPrincipal UserDetails userDetails,
        @Parameter(name = "id", description = "수정하려는 주소의 id", in = ParameterIn.PATH) @PathVariable Long id, @RequestBody AddressDTO addressDto){
    ...
    public ResponseEntity<String> deleteMemberAddress(@AuthenticationPrincipal UserDetails userDetails,
        @Parameter(name = "id", description = "삭제하려는 주소의 id", in = ParameterIn.PATH) @PathVariable Long id){
